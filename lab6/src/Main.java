...
public class Main extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        for (Button btn : new Button[]{btnAdd, btnDel, btnCalc}) {
            btn.setMaxWidth(Double.MAX_VALUE);
        }
        for (Button btn : new Button[]{btnClear, btnFill, btnSaveText, btnLoadText,
                btnSaveBin, btnLoadBin, btnSaveJson, btnLoadJson}) {
        ...
        btnCalc.setOnAction(e -> {
            Table selected = tableView.getSelectionModel().getSelectedItem();
            if (selected == null) {
                showAlert("Внимание", "Выберите строку в таблице для вычисления.");
                return;
            }
            ...
            if (low >= up) {
                showAlert("Ошибка", "Нижний предел должен быть меньше верхнего.");
                return;
            }
            if (step <= 0 || step >= up) {
                showAlert("Ошибка", "Шаг должен быть положительным и меньше верхнего предела.");
                return;
            }

            Task<Void> distributedTask = new Task<>() {
                @Override
                protected Void call() throws Exception {
                    double total = distributedIntegral(low, up, step);
                    javafx.application.Platform.runLater(() -> {
                        selected.resultProperty().set(total);
                        if (selected.getRecIntegral() != null) {
                            selected.getRecIntegral().setResult(total);
                        }
                        tableView.refresh();
                    });
                    return null;
            ...
            new Thread(distributedTask).start();
        });
    ...
    private double distributedIntegral(double lower, double upper, double step) throws Exception {
        double subLen = (upper - lower) / CLIENTS_COUNT;
        double[][] subintervals = new double[CLIENTS_COUNT][2];
        for (int i = 0; i < CLIENTS_COUNT; i++) {
            subintervals[i][0] = lower + i * subLen;
            subintervals[i][1] = (i == CLIENTS_COUNT - 1) ? upper : lower + (i + 1) * subLen;
        }

        try (ServerSocket serverSocket = new ServerSocket(SERVER_PORT)) {
            // Запускаем 7 клиентских процессов
            Process[] clientProcesses = new Process[CLIENTS_COUNT];
            for (int i = 0; i < CLIENTS_COUNT; i++) {
                String classpath = System.getProperty("java.class.path");
                ProcessBuilder pb = new ProcessBuilder("java", "-cp", classpath, "Client", "localhost", String.valueOf(SERVER_PORT));
                pb.inheritIO(); // вывод клиентов в консоль
                clientProcesses[i] = pb.start();
            }

            double[] partialResults = new double[CLIENTS_COUNT];
            ...
            for (int i = 0; i < CLIENTS_COUNT; i++) {
                ...
                new Thread(() -> {
                    try (DataOutputStream dos = new DataOutputStream(clientSocket.getOutputStream());
                        ...
                        dos.writeDouble(subintervals[index][0]);
                        dos.writeDouble(subintervals[index][1]);
                        dos.writeDouble(step);
                        dos.flush();
                        double res = dis.readDouble();
                        partialResults[index] = res;
                        latch.countDown();
                    } catch (IOException ex) {
                        ex.printStackTrace();
                        latch.countDown();
            ...
            latch.await();
            for (Process p : clientProcesses) {
                if (p.isAlive()) p.destroyForcibly();
            }

            double total = 0.0;
            for (double v : partialResults) total += v;
            return total;
        }
    }
